...
#include "../../plotting/datastructures/plotrow.h"
...
const std::string LP_Plot::loggerCat_("voreen.LP_Plot");

LP_Plot::LP_Plot()
    : PlotProcessor(PlotEntitySettings::LINE, false)
    , textInport_(Port::INPORT,"text")
    ...
    , pickingList_(0)
{
    ...
    logXAxis_.onChange       (CallMemberAction<LP_Plot>(this, &LP_Plot::regenDisplayLists));
    logYAxis_.onChange       (CallMemberAction<LP_Plot>(this, &LP_Plot::regenDisplayLists));
    renderLineLabel_.onChange(CallMemberAction<LP_Plot>(this, &LP_Plot::regenDisplayLists));
    lineWidth_.onChange      (CallMemberAction<LP_Plot>(this, &LP_Plot::regenDisplayLists));
    pointSize_.onChange      (CallMemberAction<LP_Plot>(this, &LP_Plot::regenDisplayLists));

}



Processor* LP_Plot::create() const {
    return new LP_Plot();
}
...
bool LP_Plot::isReady() const {
    if (!isInitialized() || !inport_.isReady() || !outport_.isReady())
        return false;
    return true;
}
...
void LP_Plot::readFromInport() {
    ...
    if (dynamic_cast<const PlotData*>(inport_.getData())) {
        ...
        LWARNINGC("LP_Plot", "LP_Plot can only handle PlotData objects with one key column");
        data_ = PlotData(0,0);
        dataProp_.set(&data_);
        plotEntitiesProp_.setPlotData(&data_);
        selectionProp_.setPlotData(&data_);
