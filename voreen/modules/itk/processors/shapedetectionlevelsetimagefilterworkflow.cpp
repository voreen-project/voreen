...
#include "shapedetectionlevelsetimagefilterworkflow.h"
...
const std::string ShapeDetectionLevelSetImageFilterWorkflowITK::loggerCat_("voreen.ShapeDetectionLevelSetImageFilterWorkflowITK");

ShapeDetectionLevelSetImageFilterWorkflowITK::ShapeDetectionLevelSetImageFilterWorkflowITK()
    : ITKProcessor(),
    ...
    propagationScaling_("propagationScaling", "PropagationScaling", 2.0f, -50.0f, 50.0f),
    curvatureScaling_("curvatureScaling", "CurvatureScaling", 2.0f, 0.0f, 50.0f)
{
    addPort(inport1_);
    ...
    numSeedPoint_.onChange(MemberFunctionCallback<ShapeDetectionLevelSetImageFilterWorkflowITK>(this, &ShapeDetectionLevelSetImageFilterWorkflowITK::process));
    seedPoint_.setReadOnlyFlag(!false);
    ...
    curvatureScaling_.setGroupID("segmentation");
    setPropertyGroupGuiName("segmentation", "ShapeDetectionLevelSet");
}

Processor* ShapeDetectionLevelSetImageFilterWorkflowITK::create() const {
    return new ShapeDetectionLevelSetImageFilterWorkflowITK();
}



template<class T>
void ShapeDetectionLevelSetImageFilterWorkflowITK::shapeDetectionLevelSetImageFilterWorkflowITK() {

    inputIsBinary_.setVisibleFlag(true);
    squaredDistance_.setVisibleFlag(true);
    useImageSpacing_.setVisibleFlag(true);
    ...
    //LevelsetFilter
    typedef itk::ShapeDetectionLevelSetImageFilter<InputImageType, InputImageType> LevelType;
    ...
    if (initLevelSet_.isSelected("danielsson")){
        ...
        levelfilter->SetCurvatureScaling( curvatureScaling_.get() );
        ...
        inputIsBinary_.setVisibleFlag(false);
        squaredDistance_.setVisibleFlag(false);
        useImageSpacing_.setVisibleFlag(false);
        ...
        levelfilter2->SetCurvatureScaling( curvatureScaling_.get() );
...
void ShapeDetectionLevelSetImageFilterWorkflowITK::process() {
    ...
    if (dynamic_cast<const VolumeRAM_Float*>(inputVolume1))  {
        shapeDetectionLevelSetImageFilterWorkflowITK<float>();
    }
    else {
