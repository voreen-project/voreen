					   ...
					   int outputIsBinary )
{
  ...
  if ( components == (typeConnectedComponent *)NULL ) {
    if ( (typeOut != USHORT_TYPE) && (typeOut != SSHORT) ) free( tmpBuf );
    if ( _VERBOSE_)
  ...
  if ( nbFoundCC <= 0 ) {
    ...
    case SCHAR_TYPE :
      break;
    case USHORT_TYPE :   v *= sizeof( u16 );   break;
    case SSHORT :   v *= sizeof( s16 );   break;
    case FLOAT_TYPE  :   v *= sizeof( r32 );   break;
    default :
      if ( _VERBOSE_ ) {
...
{
  const char *proc = "InternalConnectedComponentsExtraction";
  /* sizes 
   */
  int dimx = theDim[0];
  int dimy = theDim[1];
...
static typeConnectedComponent* LabelsOverflowManagement( unsigned short int *inputBuf,
							 ...
							 */
							 ...
							 int x, int y, int z )
{
  const char *proc="_LabelsOverflowManagement";
...
static int RelabelConnectedComponents( void *inputBuf,
				       ...
				       int binaryLabel )
{
  const char *proc = "RelabelConnectedComponents";
                     ...
                     int decreasing)
{
  ...
  SortCCWithRespectToSize( tab, left, last-1, decreasing);
  SortCCWithRespectToSize( tab, last+1, right, decreasing);
}
...
int RelabelConnectedComponentsBySize( void *inputBuf,
						bufferType typeIn,
						int *theDim,
                        int decreasing)
{
  const char *proc = "RelabelConnectedComponentsByDecreasingSize";
  int i, v;
  int lmax = 0;
  ...
  SortCCWithRespectToSize( theCC, 1, lmax, decreasing);
