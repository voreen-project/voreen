...
#ifndef SUPER_AVERAGE_2D_HH
#define SUPER_AVERAGE_2D_HH
...
SuperAverage2D<T,W>::SuperAverage2D(FunctorPtr<SuperF2D<T,W>>&&        f,
  ...
  if ( _f->getBlockFSize()          == load.size() &&
    ...
    for (int iC = 0; iC < load.size(); ++iC) {
      this->_blockF.emplace_back(
        new BlockAverage2D<T,W>(_f->getBlockF(iC),
...
template <typename T, typename W>
bool SuperAverage2D<T,W>::operator() (W output[], const int input[])
{
