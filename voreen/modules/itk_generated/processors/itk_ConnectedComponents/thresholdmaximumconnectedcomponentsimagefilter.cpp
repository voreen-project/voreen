...
#include "thresholdmaximumconnectedcomponentsimagefilter.h"
...
const std::string ThresholdMaximumConnectedComponentsImageFilterITK::loggerCat_("voreen.ThresholdMaximumConnectedComponentsImageFilterITK");

ThresholdMaximumConnectedComponentsImageFilterITK::ThresholdMaximumConnectedComponentsImageFilterITK()
    : ITKProcessor(),
    inport1_(Port::INPORT, "InputImage"),
    outport1_(Port::OUTPORT, "OutputImage"),
    enableProcessing_("enabled", "Enable", false),
    minimumObjectSizeInPixels_("minimumObjectSizeInPixels", "MinimumObjectSizeInPixels", 0, 0, INT_MAX),
    insideValue_("insideValue", "InsideValue"),
    outsideValue_("outsideValue", "OutsideValue"),
    upperBoundary_("upperBoundary", "UpperBoundary"),
    thresholdValue_("thresholdValue", "ThresholdValue"),
    numberOfObjects_("numberOfObjects", "NumberOfObjects", 0, INT_MIN, INT_MAX)
{
    ...
    addProperty(minimumObjectSizeInPixels_);
    addProperty(insideValue_);
    addProperty(outsideValue_);
    addProperty(upperBoundary_);
    addProperty(thresholdValue_);
    thresholdValue_.setReadOnlyFlag(true);
    addProperty(numberOfObjects_);
    numberOfObjects_.setReadOnlyFlag(true);

}

Processor* ThresholdMaximumConnectedComponentsImageFilterITK::create() const {
    return new ThresholdMaximumConnectedComponentsImageFilterITK();
}

template<class T>
void ThresholdMaximumConnectedComponentsImageFilterITK::thresholdMaximumConnectedComponentsImageFilterITK() {
    insideValue_.setVolume(inport1_.getData());
    outsideValue_.setVolume(inport1_.getData());
    upperBoundary_.setVolume(inport1_.getData());
    thresholdValue_.setVolume(inport1_.getData());
    ...
    //Filter define
    typedef itk::ThresholdMaximumConnectedComponentsImageFilter<InputImageType1, OutputImageType1> FilterType;
    typename FilterType::Pointer filter = FilterType::New();

    filter->SetInput(p1);

    filter->SetMinimumObjectSizeInPixels(minimumObjectSizeInPixels_.get());
    filter->SetInsideValue(insideValue_.getValue<T>());
    filter->SetOutsideValue(outsideValue_.getValue<T>());
    filter->SetUpperBoundary(upperBoundary_.getValue<T>());
    ...
    {
        filter->Update();
        thresholdValue_.setValue<T>(filter->GetThresholdValue());
...
void ThresholdMaximumConnectedComponentsImageFilterITK::process() {
    ...
    if (dynamic_cast<const VolumeRAM_UInt8*>(inputVolume1))  {
        thresholdMaximumConnectedComponentsImageFilterITK<uint8_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int8*>(inputVolume1))  {
        thresholdMaximumConnectedComponentsImageFilterITK<int8_t>();
    }
    else if (dynamic_cast<const VolumeRAM_UInt16*>(inputVolume1))  {
        thresholdMaximumConnectedComponentsImageFilterITK<uint16_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int16*>(inputVolume1))  {
        thresholdMaximumConnectedComponentsImageFilterITK<int16_t>();
    }
    else if (dynamic_cast<const VolumeRAM_UInt32*>(inputVolume1))  {
        thresholdMaximumConnectedComponentsImageFilterITK<uint32_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int32*>(inputVolume1))  {
        thresholdMaximumConnectedComponentsImageFilterITK<int32_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Float*>(inputVolume1))  {
        thresholdMaximumConnectedComponentsImageFilterITK<float>();
    }
    else if (dynamic_cast<const VolumeRAM_Double*>(inputVolume1))  {
        thresholdMaximumConnectedComponentsImageFilterITK<double>();
    }
    else {
