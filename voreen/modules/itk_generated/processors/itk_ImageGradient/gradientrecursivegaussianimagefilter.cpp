...
#include "gradientrecursivegaussianimagefilter.h"
...
const std::string GradientRecursiveGaussianImageFilterITK::loggerCat_("voreen.GradientRecursiveGaussianImageFilterITK");

GradientRecursiveGaussianImageFilterITK::GradientRecursiveGaussianImageFilterITK()
    : ITKProcessor(),
    ...
    useImageDirection_("useImageDirection", "UseImageDirection", false),
    sigma_("sigma", "Sigma", 0.1f, 0.0f, 1.0f)
{
    ...
    addProperty(useImageDirection_);
    addProperty(sigma_);

}

Processor* GradientRecursiveGaussianImageFilterITK::create() const {
    return new GradientRecursiveGaussianImageFilterITK();
}
...
void GradientRecursiveGaussianImageFilterITK::process() {
    ...
    if (dynamic_cast<const VolumeRAM_Float*>(inputVolume1))  {
        gradientRecursiveGaussianImageFilterITK<float>();
    }
    else if (dynamic_cast<const VolumeRAM_Double*>(inputVolume1))  {
        gradientRecursiveGaussianImageFilterITK<double>();
    }
    else {
