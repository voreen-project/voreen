...
#include "signeddanielssondistancemapimagefilter.h"
...
const std::string SignedDanielssonDistanceMapImageFilterITK::loggerCat_("voreen.SignedDanielssonDistanceMapImageFilterITK");

SignedDanielssonDistanceMapImageFilterITK::SignedDanielssonDistanceMapImageFilterITK()
    : ITKProcessor(),
    inport1_(Port::INPORT, "InputImage"),
    outport1_(Port::OUTPORT, "DistanceMap"),
    outport2_(Port::OUTPORT, "VoronoiMap"),
    insideIsPositive_("insideIsPositive", "InsideIsPositive", false),
    ...
    useImageSpacing_("useImageSpacing", "UseImageSpacing", false)
{
    ...
    addProperty(insideIsPositive_);
    addProperty(squaredDistance_);
    addProperty(useImageSpacing_);

}

Processor* SignedDanielssonDistanceMapImageFilterITK::create() const {
    return new SignedDanielssonDistanceMapImageFilterITK();
}
...
void SignedDanielssonDistanceMapImageFilterITK::process() {
    ...
    if (dynamic_cast<const VolumeRAM_UInt8*>(inputVolume1))  {
        signedDanielssonDistanceMapImageFilterITK<uint8_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int8*>(inputVolume1))  {
        signedDanielssonDistanceMapImageFilterITK<int8_t>();
    }
    else if (dynamic_cast<const VolumeRAM_UInt16*>(inputVolume1))  {
        signedDanielssonDistanceMapImageFilterITK<uint16_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int16*>(inputVolume1))  {
        signedDanielssonDistanceMapImageFilterITK<int16_t>();
    }
    else if (dynamic_cast<const VolumeRAM_UInt32*>(inputVolume1))  {
        signedDanielssonDistanceMapImageFilterITK<uint32_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int32*>(inputVolume1))  {
        signedDanielssonDistanceMapImageFilterITK<int32_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Float*>(inputVolume1))  {
        signedDanielssonDistanceMapImageFilterITK<float>();
    }
    else if (dynamic_cast<const VolumeRAM_Double*>(inputVolume1))  {
        signedDanielssonDistanceMapImageFilterITK<double>();
    }
    else {
