...
#include "binarypruningimagefilter.h"
...
const std::string BinaryPruningImageFilterITK::loggerCat_("voreen.BinaryPruningImageFilterITK");

BinaryPruningImageFilterITK::BinaryPruningImageFilterITK()
    : ITKProcessor(),
    inport1_(Port::INPORT, "InputImage"),
    outport1_(Port::OUTPORT, "OutputImage"),
    enableProcessing_("enabled", "Enable", false),
    iteration_("iteration", "Iteration", 1, 1, 100)
{
    addPort(inport1_);
    ...
    addProperty(iteration_);

}

Processor* BinaryPruningImageFilterITK::create() const {
    return new BinaryPruningImageFilterITK();
}

template<class T>
void BinaryPruningImageFilterITK::binaryPruningImageFilterITK() {
    ...
    //Filter define
    typedef itk::BinaryPruningImageFilter<InputImageType1, OutputImageType1> FilterType;
    typename FilterType::Pointer filter = FilterType::New();

    filter->SetInput(p1);

    filter->SetIteration(iteration_.get());
...
void BinaryPruningImageFilterITK::process() {
