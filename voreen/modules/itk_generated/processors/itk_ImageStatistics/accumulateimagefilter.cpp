...
#include "accumulateimagefilter.h"
...
const std::string AccumulateImageFilterITK::loggerCat_("voreen.AccumulateImageFilterITK");

AccumulateImageFilterITK::AccumulateImageFilterITK()
    : ITKProcessor(),
    inport1_(Port::INPORT, "InputImage"),
    outport1_(Port::OUTPORT, "OutputImage"),
    enableProcessing_("enabled", "Enable", false),
    accumulateDimension_("accumulateDimension", "AccumulateDimension", 1, 1, 10)
{
    addPort(inport1_);
    ...
    addProperty(accumulateDimension_);

}

Processor* AccumulateImageFilterITK::create() const {
    return new AccumulateImageFilterITK();
}

template<class T>
void AccumulateImageFilterITK::accumulateImageFilterITK() {
    ...
    //Filter define
    typedef itk::AccumulateImageFilter<InputImageType1, OutputImageType1> FilterType;
    typename FilterType::Pointer filter = FilterType::New();

    filter->SetInput(p1);

    filter->SetAccumulateDimension(accumulateDimension_.get());
...
void AccumulateImageFilterITK::process() {
