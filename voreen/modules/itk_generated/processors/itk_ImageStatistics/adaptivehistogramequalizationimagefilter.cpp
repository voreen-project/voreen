...
#include "adaptivehistogramequalizationimagefilter.h"
...
const std::string AdaptiveHistogramEqualizationImageFilterITK::loggerCat_("voreen.AdaptiveHistogramEqualizationImageFilterITK");

AdaptiveHistogramEqualizationImageFilterITK::AdaptiveHistogramEqualizationImageFilterITK()
    : ITKProcessor(),
    inport1_(Port::INPORT, "InputImage"),
    outport1_(Port::OUTPORT, "OutputImage"),
    enableProcessing_("enabled", "Enable", false),
    alpha_("alpha", "Alpha", 0.0f, 0.0f, 1.0f),
    beta_("beta", "Beta", 0.0f, 0.0f, 1.0f)
{
    addPort(inport1_);
...
Processor* AdaptiveHistogramEqualizationImageFilterITK::create() const {
    return new AdaptiveHistogramEqualizationImageFilterITK();
}

template<class T>
void AdaptiveHistogramEqualizationImageFilterITK::adaptiveHistogramEqualizationImageFilterITK() {
    ...
    //Filter define
    typedef itk::AdaptiveHistogramEqualizationImageFilter<InputImageType1> FilterType;
...
void AdaptiveHistogramEqualizationImageFilterITK::process() {
    ...
    if (dynamic_cast<const VolumeRAM_UInt8*>(inputVolume1))  {
        adaptiveHistogramEqualizationImageFilterITK<uint8_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int8*>(inputVolume1))  {
        adaptiveHistogramEqualizationImageFilterITK<int8_t>();
    }
    else if (dynamic_cast<const VolumeRAM_UInt16*>(inputVolume1))  {
        adaptiveHistogramEqualizationImageFilterITK<uint16_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int16*>(inputVolume1))  {
        adaptiveHistogramEqualizationImageFilterITK<int16_t>();
    }
    else if (dynamic_cast<const VolumeRAM_UInt32*>(inputVolume1))  {
        adaptiveHistogramEqualizationImageFilterITK<uint32_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int32*>(inputVolume1))  {
        adaptiveHistogramEqualizationImageFilterITK<int32_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Float*>(inputVolume1))  {
        adaptiveHistogramEqualizationImageFilterITK<float>();
    }
    else if (dynamic_cast<const VolumeRAM_Double*>(inputVolume1))  {
        adaptiveHistogramEqualizationImageFilterITK<double>();
    }
    else {
