...
#include "getaveragesliceimagefilter.h"
...
const std::string GetAverageSliceImageFilterITK::loggerCat_("voreen.GetAverageSliceImageFilterITK");

GetAverageSliceImageFilterITK::GetAverageSliceImageFilterITK()
    : ITKProcessor(),
    inport1_(Port::INPORT, "InputImage"),
    outport1_(Port::OUTPORT, "OutputImage"),
    enableProcessing_("enabled", "Enable", false),
    averagedOutDimension_("averagedOutDimension", "AveragedOutDimension", 0, 0, 1)
{
    addPort(inport1_);
    ...
    addProperty(averagedOutDimension_);

}

Processor* GetAverageSliceImageFilterITK::create() const {
    return new GetAverageSliceImageFilterITK();
}

template<class T>
void GetAverageSliceImageFilterITK::getAverageSliceImageFilterITK() {
    ...
    //Filter define
    typedef itk::GetAverageSliceImageFilter<InputImageType1, OutputImageType1> FilterType;
    typename FilterType::Pointer filter = FilterType::New();

    filter->SetInput(p1);

    filter->SetAveragedOutDimension(averagedOutDimension_.get());
...
void GetAverageSliceImageFilterITK::process() {
