...
#include "labelvotingimagefilter.h"
...
const std::string LabelVotingImageFilterITK::loggerCat_("voreen.LabelVotingImageFilterITK");

LabelVotingImageFilterITK::LabelVotingImageFilterITK()
    : ITKProcessor(),
    inport1_(Port::INPORT, "InputImage"),
    outport1_(Port::OUTPORT, "OutputImage"),
    enableProcessing_("enabled", "Enable", false),
    labelForUndecidedPixels_("labelForUndecidedPixels", "LabelForUndecidedPixels")
{
    addPort(inport1_);
    ...
    addProperty(labelForUndecidedPixels_);

}

Processor* LabelVotingImageFilterITK::create() const {
    return new LabelVotingImageFilterITK();
}

template<class T>
void LabelVotingImageFilterITK::labelVotingImageFilterITK() {
    labelForUndecidedPixels_.setVolume(inport1_.getData());
    ...
    //Filter define
    typedef itk::LabelVotingImageFilter<InputImageType1, OutputImageType1> FilterType;
    typename FilterType::Pointer filter = FilterType::New();

    filter->SetInput(p1);

    filter->SetLabelForUndecidedPixels(labelForUndecidedPixels_.getValue<T>());
...
void LabelVotingImageFilterITK::process() {
