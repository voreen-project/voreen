...
#include "votingbinaryiterativeholefillingimagefilter.h"
...
const std::string VotingBinaryIterativeHoleFillingImageFilterITK::loggerCat_("voreen.VotingBinaryIterativeHoleFillingImageFilterITK");

VotingBinaryIterativeHoleFillingImageFilterITK::VotingBinaryIterativeHoleFillingImageFilterITK()
    : ITKProcessor(),
    inport1_(Port::INPORT, "InputImage"),
    outport1_(Port::OUTPORT, "OutputImage"),
    enableProcessing_("enabled", "Enable", false),
    maximumNumberOfIterations_("maximumNumberOfIterations", "MaximumNumberOfIterations", 1, 1, 10000)
{
    addPort(inport1_);
    ...
    addProperty(maximumNumberOfIterations_);

}

Processor* VotingBinaryIterativeHoleFillingImageFilterITK::create() const {
    return new VotingBinaryIterativeHoleFillingImageFilterITK();
}
...
void VotingBinaryIterativeHoleFillingImageFilterITK::process() {
    ...
    if (dynamic_cast<const VolumeRAM_UInt8*>(inputVolume1))  {
        votingBinaryIterativeHoleFillingImageFilterITK<uint8_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int8*>(inputVolume1))  {
        votingBinaryIterativeHoleFillingImageFilterITK<int8_t>();
    }
    else if (dynamic_cast<const VolumeRAM_UInt16*>(inputVolume1))  {
        votingBinaryIterativeHoleFillingImageFilterITK<uint16_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int16*>(inputVolume1))  {
        votingBinaryIterativeHoleFillingImageFilterITK<int16_t>();
    }
    else if (dynamic_cast<const VolumeRAM_UInt32*>(inputVolume1))  {
        votingBinaryIterativeHoleFillingImageFilterITK<uint32_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Int32*>(inputVolume1))  {
        votingBinaryIterativeHoleFillingImageFilterITK<int32_t>();
    }
    else if (dynamic_cast<const VolumeRAM_Float*>(inputVolume1))  {
        votingBinaryIterativeHoleFillingImageFilterITK<float>();
    }
    else if (dynamic_cast<const VolumeRAM_Double*>(inputVolume1))  {
        votingBinaryIterativeHoleFillingImageFilterITK<double>();
    }
    else {
