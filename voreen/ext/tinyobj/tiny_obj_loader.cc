...
/// new: use case insensitive tag comparison
int strncmpCI (const char *p1, const char *p2, int length) {
  unsigned char *s1 = (unsigned char *) p1;
  unsigned char *s2 = (unsigned char *) p2;

  for(int i = 0; i < length; i++) {
      unsigned char c1 = (unsigned char) toupper((int)*s1++);
      unsigned char c2 = (unsigned char) toupper((int)*s2++);
      if (c1 != c2)
        return c1 - c2;
  }
  ...
  const char* mtl_basepath)
{
  ...
  while (ifs.peek() != -1) {
    ...
    // new mtl
    if ((0 == strncmpCI(token, "newmtl", 6)) && isSpace((token[6]))) {
    ...
    // ambient texture
    if ((0 == strncmpCI(token, "map_Ka", 6)) && isSpace(token[6])) {
      token += 7;
    ...
    // diffuse texture
    if ((0 == strncmpCI(token, "map_Kd", 6)) && isSpace(token[6])) {
      token += 7;
    ...
    // specular texture
    if ((0 == strncmpCI(token, "map_Ks", 6)) && isSpace(token[6])) {
      token += 7;
    ...
    // normal texture
    if ((0 == strncmpCI(token, "map_Ns", 6)) && isSpace(token[6])) {
      token += 7;
      material.normal_texname = token;
      continue;
    }

    ///// new: save bumpmapping filename
    // bump texture
    if ((0 == strncmpCI(token, "bump", 4)) && isSpace(token[4])) {
      token += 5;
  ...
  const char* mtl_basepath)
{
  ...
  while (ifs.peek() != -1) {
    ...
    // use mtl
    if ((0 == strncmpCI(token, "usemtl", 6)) && isSpace((token[6]))) {
      ////////////////////////// new: flush current material faces to separate shape
    ...
    // load mtl
    if ((0 == strncmpCI(token, "mtllib", 6)) && isSpace((token[6]))) {
      char namebuf[4096];
      token += 7;
      //// new: allow for spaces in mtl filenames
      sscanf(token, "%[^\n\r]", namebuf);
      //// new
