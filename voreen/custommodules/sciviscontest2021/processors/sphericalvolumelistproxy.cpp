...
#include "sphericalvolumelistproxy.h"
...
const std::string SphericalVolumeListProxy::loggerCat_("voreen.SphericalVolumeListProxy");

SphericalVolumeListProxy::SphericalVolumeListProxy()
...
Processor* SphericalVolumeListProxy::create() const {
    return new SphericalVolumeListProxy();
}

void SphericalVolumeListProxy::process() {

    proxyVolumes_.clear();

    auto inputVolumeList = inport_.getData();
    auto outputVolumeList = new VolumeList();
    ...
    for(size_t i=0; i<inputVolumeList->size(); i++) {

        auto inputVolume = inputVolumeList->at(i);
        ...
        outputVolumeList->add(output);
        proxyVolumes_.emplace_back(std::unique_ptr<VolumeBase>(output));
    }

    outport_.setData(outputVolumeList);
}

void SphericalVolumeListProxy::afterConnectionAdded(const Port* source, const Port* connectedPort) {
}
void SphericalVolumeListProxy::beforeConnectionRemoved(const Port* source, const Port*) {
}

void SphericalVolumeListProxy::dataWillChange(const Port* source) {
    outport_.clear();
...
void SphericalVolumeListProxy::dataHasChanged(const Port* source) {
    if(source->hasData()) {
        source->addDataInvalidationObserver(static_cast<DataInvalidationObserver*>(this));
    }
}

void SphericalVolumeListProxy::dataAboutToInvalidate(const DataInvalidationObservable* data) {
    outport_.clear();
}
