...
#include "sphericalcuttingplaneselector.h"
...
SphericalCuttingPlaneSelector::SphericalCuttingPlaneSelector()
    ...
    , mouseEventProp_("mouseEvent.measure", "Distance measure", this, &SphericalCuttingPlaneSelector::measure, tgt::MouseEvent::MOUSE_BUTTON_LEFT, tgt::MouseEvent::MOTION | tgt::MouseEvent::PRESSED | tgt::MouseEvent::RELEASED, tgt::Event::CTRL, false)
    , resetEventProp_("mouseEvent.reset", "Reset", this, &SphericalCuttingPlaneSelector::resetClipping, tgt::MouseEvent::MOUSE_BUTTON_RIGHT, tgt::MouseEvent::PRESSED, tgt::Event::CTRL, false)
    ...
    , autoUpdate_("autoUpdate", "Update plane on motion", false)
    ...
    , material_()       //
{
    ...
    addEventProperty(&resetEventProp_);
...
Processor* SphericalCuttingPlaneSelector::create() const {
    return new SphericalCuttingPlaneSelector();
}

bool SphericalCuttingPlaneSelector::isReady() const {
...
tgt::ivec2 SphericalCuttingPlaneSelector::clampToViewport(tgt::ivec2 mousePos) {
...
void SphericalCuttingPlaneSelector::measure(tgt::MouseEvent* e) {
    ...
    if (e->action() & tgt::MouseEvent::MOTION) {
        if (mouseDown_) {
            ...
            if (length(fhp) > 0.0f) {
                ...
                float r = radiusMax_.get();
                distance_ = r * std::acos(tgt::dot(mouseStartPos3D_, mouseCurPos3D_) / (r*r));

                if(autoUpdate_.get()) {
                    enableClipping_.set(true);
                    updatePlane();
                }
...
void SphericalCuttingPlaneSelector::updatePlane() {

    tgt::vec3 start = mouseStartPos3D_.xyz();
    tgt::vec3 end = mouseCurPos3D_.xyz();
    tgt::vec3 center = tgt::vec3::zero;

    tgt::plane plane(center, end, start);
    planeNormal_.set(plane.n);
    planeDistance_.set(plane.d);
}

void SphericalCuttingPlaneSelector::resetClipping(tgt::MouseEvent* e) {
    enableClipping_.set(false);
    e->accept();
}

void SphericalCuttingPlaneSelector::process() {
