...
#include "geometrysequencesource.h"
...
namespace voreen {
    ...
    const std::string GeometrySequenceSource::loggerCat_("voreen.core.GeometrySequenceSource");

    GeometrySequenceSource::GeometrySequenceSource()
        : Processor()
        , geometryDirectory_("geometryDirectory", "Geometry Directory", "Open Geometry Directory", 
            VoreenApplication::app()->getUserDataPath(), 
            "Geometry (*.vge *.ply *.obj *.stl);;Voreen Geometry (*.vge);;PLY mesh (*.ply);;OBJ mesh (*.obj);;STL mesh (*.stl);;ASCII point lists (*.txt)",
        ...
        , forceReload_(false)
    {
        ...
        geometryDirectory_.onChange(MemberFunctionCallback<GeometrySequenceSource>(this, &GeometrySequenceSource::forceReload));
        geometryType_.onChange(MemberFunctionCallback<GeometrySequenceSource>(this, &GeometrySequenceSource::updatePropertyVisibility));
        loadGeometry_.onChange(MemberFunctionCallback<GeometrySequenceSource>(this, &GeometrySequenceSource::forceReload));
        clearGeometry_.onChange(MemberFunctionCallback<GeometrySequenceSource>(this, &GeometrySequenceSource::clearGeometry));

        addProperty(geometryDirectory_);
    ...
    Processor* GeometrySequenceSource::create() const {
        return new GeometrySequenceSource();
    }

    void GeometrySequenceSource::process() {
        if (geometryDirectory_.get() != "" && forceReload_) {
            try {
                readGeometry();
            }
    ...
    void GeometrySequenceSource::initialize() {
        Processor::initialize();
        forceReload_ = true;
    }

    void GeometrySequenceSource::readGeometry() {

        std::set<std::string> supportedFormats = {"vge", "obj", "stl", "ply"};

        // Iterate over list and find all vge files
        ...
        for(auto filename : filenames) {
            if(supportedFormats.find(tgt::FileSystem::fileExtension(filename, true)) != supportedFormats.end()) {
                geometryFilenames.push_back(filename);
            }
        } 

        size_t idx = 0;
        GeometrySequence* geometries = new GeometrySequence(true); // take ownership
        for (auto geometryFilename : geometryFilenames) {
            std::string filename = filedirectory + "/" + geometryFilename;
            LINFO("Load " + filename);
            ...
            if (geometryType_.isSelected("geometry")) {
                ...
                if (geometryType_.isSelected("pointlist")) {
                    LINFO("Reading point list from file: " << filename);
                    listType = PointList;
                }
                else {
                    LINFO("Reading segmented point list from file: " << filename);
                    listType = SegmentedPointList;
                }

                try {
                    Geometry* geometry = readPointList(filename, listType, skipItemCount_.get());
                    tgtAssert(geometry, "null pointer returned (exception expected)");
                    geometries->addGeometry(geometry);
                }
                catch (VoreenException & e) {
        ...
        outport_.setData(geometries);
        updatePropertyVisibility();
    }

    Geometry* GeometrySequenceSource::readPLYGeometry(const std::string& filename) {
...
#pragma omp parallel for
    ...
    Geometry* GeometrySequenceSource::readOBJGeometry(const std::string& filename) {
    ...
    Geometry* GeometrySequenceSource::readOBJGeometryWithColor(const std::string& filename, tgt::vec4 color) {
        std::vector<tinyobj::shape_t> shapes;
        std::string basePath = tgt::FileSystem::dirName(filename) + "/";
    ...
    Geometry* GeometrySequenceSource::readSTLGeometry(const std::string& filename) {
    ...
    Geometry* GeometrySequenceSource::readVoreenGeometry(const std::string& filename) {
        ...
        try {
            ...
            throw VoreenException("Failed to deserialize Voreen Geometry from " + geometryDirectory_.get() + ": " + e.what());
        }
    }

    Geometry* GeometrySequenceSource::readPointList(const std::string& filename, PointListType listType, int skipItems) {
    ...
    void GeometrySequenceSource::clearGeometry() {
        outport_.setData(0);
        geometryDirectory_.set("");
        updatePropertyVisibility();
    }

    void GeometrySequenceSource::forceReload() {
        forceReload_ = true;
        invalidate();
    }

    void GeometrySequenceSource::updatePropertyVisibility() {
        loadGeometry_.setReadOnlyFlag(geometryDirectory_.get() == "");
