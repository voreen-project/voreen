...
struct FreeSurfaceAppHelper {
  ...
  std::array<T,2> initial_falling_speed{{0., -6.03}};
};
...
template <typename T, typename DESCRIPTOR>
class FallingDropVel2D final : public AnalyticalF<2,T,T> {
private:
  T lattice_size;
  std::array<T,DESCRIPTOR::d> lattice_speed;
public:
  FallingDropVel2D(T lattice_size_, const std::array<T,DESCRIPTOR::d>& lattice_speed_):AnalyticalF<2,T,T>{2}, lattice_size{lattice_size_}, lattice_speed{lattice_speed_}{}
...
void setInitialValues(SuperLattice<T, DESCRIPTOR>& sLattice, SuperGeometry<T,2>& sGeometry, T lattice_length, UnitConverter<T,DESCRIPTOR> const& converter, const FreeSurfaceAppHelper& helper){
  ...
  FallingDropVel2D<T,DESCRIPTOR> u{lattice_length, lattice_speed};
  AnalyticalConst2D<T,T> one(1.);
                 ...
                 SuperGeometry<T,2>& superGeometry, util::Timer<T>& timer)
{
  ...
  if ( iT%vtmIter==0 ) {
    ...
    SuperLatticePhysField2D<T, DESCRIPTOR, descriptors::FORCE> force(sLattice, converter.getConversionFactorForce());
