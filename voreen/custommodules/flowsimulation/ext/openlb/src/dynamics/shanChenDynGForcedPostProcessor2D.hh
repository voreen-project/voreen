...
ShanChenDynGForcedPostProcessor2D <T,Lattice>::
...
template<typename T, template<typename U> class Lattice>
void ShanChenDynGForcedPostProcessor2D<T,Lattice>::
  ...
  if ( util::intersect ( x0, x1, y0, y1,
    ...
    for (int iX=newX0; iX<=newX1; ++iX) {
      for (int iY=newY0; iY<=newY1; ++iY) {
        ...
        T *gForce = blockCell.getExternal(L::ExternalField::gBeginsAt);
...
template<typename T, template<typename U> class Lattice>
ShanChenDynGForcedGenerator2D<T,Lattice>::ShanChenDynGForcedGenerator2D (
...
template<typename T, template<typename U> class Lattice>
PostProcessor2D<T,Lattice>* ShanChenDynGForcedGenerator2D<T,Lattice>::generate (
  std::vector<SpatiallyExtendedObject2D*> partners) const
{
  return new ShanChenDynGForcedPostProcessor2D<T,Lattice>(
           this->x0,this->x1,this->y0,this->y1,G, rho0, interactionPotential, partners);
}

template<typename T, template<typename U> class Lattice>
LatticeCouplingGenerator2D<T,Lattice>* ShanChenDynGForcedGenerator2D<T,Lattice>::clone() const
{
  return new ShanChenDynGForcedGenerator2D<T,Lattice>(*this);
}
