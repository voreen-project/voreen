...
ShanChenForcedPostProcessor2D <T,Lattice>::
...
template<typename T, template<typename U> class Lattice>
void ShanChenForcedPostProcessor2D<T,Lattice>::
...
template<typename T, template<typename U> class Lattice>
ShanChenForcedGenerator2D<T,Lattice>::ShanChenForcedGenerator2D (
...
template<typename T, template<typename U> class Lattice>
PostProcessor2D<T,Lattice>* ShanChenForcedGenerator2D<T,Lattice>::generate (
  std::vector<SpatiallyExtendedObject2D*> partners) const
{
  return new ShanChenForcedPostProcessor2D<T,Lattice>(
           this->x0,this->x1,this->y0,this->y1,G, rho0, interactionPotential, partners);
}

template<typename T, template<typename U> class Lattice>
LatticeCouplingGenerator2D<T,Lattice>* ShanChenForcedGenerator2D<T,Lattice>::clone() const
{
  return new ShanChenForcedGenerator2D<T,Lattice>(*this);
}
