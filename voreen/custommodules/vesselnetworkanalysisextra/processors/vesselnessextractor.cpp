...
tgt::vec4 VesselnessFeatureExtractor::computeVesselnessFeatureVector(const SymMat3& H) const {
    ...
    if(l2 > 0 || l3 > 0) {
        ...
        if((A-l1*tgt::mat3::identity).invert(itMat)) {
            ...
            vesselDirVector = tgt::vec3::zero;
            //LWARNINGC("voreen.vesselnetworkanalysisextra.vesselnessextractor", "Singular Hesse Matrix, setting vessel dir to zero");
        }
    }
...
const std::string VesselnessExtractor::loggerCat_("voreen.vesselnetworkanalysisextra.vesselnessextractor");
