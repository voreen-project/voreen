...
void main() {
    ...
    if(colorFromDir_)
        ...
        ver01 = MVP_*vec4(pos01,1);
        ver02 = MVP_*vec4(pos02,1);
        ver03 = MVP_*vec4(pos03,1);
        ver04 = MVP_*vec4(pos04,1);
        ver05 = MVP_*vec4(pos05,1);
        ver06 = MVP_*vec4(pos06,1);
        ver07 = MVP_*vec4(pos07,1);
        ver08 = MVP_*vec4(pos08,1);
        ver09 = MVP_*vec4(pos09,1);
        ver10 = MVP_*vec4(pos10,1);
        ver11 = MVP_*vec4(pos11,1);
        ver12 = MVP_*vec4(pos12,1);
        ver13 = MVP_*vec4(pos13,1);
        
        nor01 = normalize(MVPtinv_*(-dir));
        nor02 = normalize(MVPtinv_*(    - x_dir + y_dir));
        nor03 = normalize(MVPtinv_*(    + x_dir + y_dir));
        nor04 = normalize(MVPtinv_*(dir - x_dir + y_dir));
        nor05 = normalize(MVPtinv_*(dir + x_dir + y_dir));
        nor06 = normalize(MVPtinv_*(dir - x_dir - y_dir));
        nor07 = normalize(MVPtinv_*(dir + x_dir - y_dir));
        nor08 = normalize(MVPtinv_*dir);
        nor09 = normalize(MVPtinv_*(dir         + y_dir));
        nor10 = normalize(MVPtinv_*(dir + x_dir        ));
        nor11 = normalize(MVPtinv_*(dir         - y_dir));
        nor12 = normalize(MVPtinv_*(dir - x_dir        ));
        
		
		vec4 lightPos = vec4(1.0, 0.0, 1.0, 1.0);
        //bottem
        VertexOut.normal = nor01;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos04,1)).xyz;
        gl_Position = ver04; EmitVertex();
        VertexOut.normal = nor01;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos02,1)).xyz;
        gl_Position = ver02; EmitVertex();
        VertexOut.normal = nor01;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos03,1)).xyz;
        gl_Position = ver03; EmitVertex();
        VertexOut.normal = nor01;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos01,1)).xyz;
        ...
        gl_Position = ver01; EmitVertex();
        VertexOut.normal = nor02;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos05,1)).xyz;
        gl_Position = ver05; EmitVertex();
        VertexOut.normal = nor03;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos02,1)).xyz;
        gl_Position = ver02; EmitVertex();
        VertexOut.normal = nor03;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos06,1)).xyz;
        ...
        gl_Position = ver04; EmitVertex();
        VertexOut.normal = -nor02;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos08,1)).xyz;
        ...
        gl_Position = ver03; EmitVertex();
        VertexOut.normal = -nor03;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos07,1)).xyz;
        ...
        //arrow bottem
        VertexOut.normal = nor01;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos09,1)).xyz;
        gl_Position = ver09; EmitVertex();
        VertexOut.normal = nor01;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos10,1)).xyz;
        gl_Position = ver10; EmitVertex();
        VertexOut.normal = nor01;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos11,1)).xyz;
        gl_Position = ver11; EmitVertex();
        VertexOut.normal = nor01;
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos12,1)).xyz;
        ...
        VertexOut.vertex_to_light = (lightPos - MVP_ * vec4(pos13,1)).xyz;
