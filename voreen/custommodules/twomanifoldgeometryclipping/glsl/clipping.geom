...
//#version 430 core //set in application as header to avoid NVIDIA compiler error
...
layout(location = 0) uniform vec4 plane;

//matrix for transformation to 2D
layout(location = 1) uniform mat4 xyTransform;

//indices of the xy coordinates
layout(location = 2) uniform ivec2 xyIndices;
...
#endif 
#if defined(USE_NORMAL)
    precise vec3 eNormal1;
    precise vec3 eNormal2;
#endif
#if defined(USE_TEXCOORD)
    precise vec2 eTexCoord1;
    precise vec2 eTexCoord2;
...
#endif
    transformedPos = xyTransform * ePos2;
    eOut.pos2d = vec2(transformedPos[xyIndices.x], transformedPos[xyIndices.y]);
    //eOut.pos2d = (xyTransform * ePos2/*(modelMatrix * ePos2)*/).xy;
    EmitStreamVertex(1);
...
#endif
    transformedPos = xyTransform * ePos1;
    eOut.pos2d = vec2(transformedPos[xyIndices.x], transformedPos[xyIndices.y]);
    //eOut.pos2d = (xyTransform * ePos1/*(modelMatrix * ePos1)*/).xy;
    EmitStreamVertex(1);
    EndStreamPrimitive(1);
}
