...
SaveWorkspaceMenuEntity::SaveWorkspaceMenuEntity()
    : VoreenQtMenuEntity()
{}
...
QAction* SaveWorkspaceMenuEntity::createMenuAction() {
    QAction* action = new QAction(getIcon(),QWidget::tr(getName().c_str()),0);
    action->setShortcut(QWidget::tr(getShortCut().c_str()));
    QObject::connect(action, SIGNAL(triggered()), this, SLOT(saveWorkspaceSlot()));
    return action;
}

void SaveWorkspaceMenuEntity::saveWorkspaceSlot() {
