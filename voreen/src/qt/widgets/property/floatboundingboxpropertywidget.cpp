...
FloatBoundingBoxPropertyWidget::FloatBoundingBoxPropertyWidget(FloatBoundingBoxProperty* const prop, QWidget* parent)
...
{
    xWidget_ = new FloatIntervalPropertyWidget(&xProperty_, this);
    yWidget_ = new FloatIntervalPropertyWidget(&yProperty_, this);
    zWidget_ = new FloatIntervalPropertyWidget(&zProperty_, this);
    ...
    connect(xWidget_, SIGNAL(valueChanged(tgt::vec2)), this, SLOT(valueChanged(tgt::vec2)));
    connect(yWidget_, SIGNAL(valueChanged(tgt::vec2)), this, SLOT(valueChanged(tgt::vec2)));
    connect(zWidget_, SIGNAL(valueChanged(tgt::vec2)), this, SLOT(valueChanged(tgt::vec2)));
...
FloatBoundingBoxPropertyWidget::~FloatBoundingBoxPropertyWidget() {
    delete xWidget_;
    delete yWidget_;
    delete zWidget_;
    delete xLabel_;
...
void FloatBoundingBoxPropertyWidget::updateFromPropertySlot() {
    tgt::Bounds bounds = prop_->get();
    tgt::vec3 min = bounds.getLLF();
    tgt::vec3 max = bounds.getURB();

    tgt::vec2 xminmax = tgt::vec2(min.x, max.x);
    tgt::vec2 yminmax = tgt::vec2(min.y, max.y);
    tgt::vec2 zminmax = tgt::vec2(min.z, max.z);

    tgt::vec3 minValue = prop_->getMinValue();
...
void FloatBoundingBoxPropertyWidget::valueChanged(tgt::vec2){
    tgt::vec2 xminmax = xProperty_.get();
    tgt::vec2 yminmax = yProperty_.get();
    tgt::vec2 zminmax = zProperty_.get();

    tgt::vec3 min = tgt::vec3(xminmax.x, yminmax.x, zminmax.x);
    tgt::vec3 max = tgt::vec3(xminmax.y, yminmax.y, zminmax.y);
