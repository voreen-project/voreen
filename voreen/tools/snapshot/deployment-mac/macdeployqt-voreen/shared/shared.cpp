...
{
    return ((a.dylibPath == b.dylibPath));
}
...
{
    debug << "Dylib name" << info.dylibName << "\n";
    debug << "Dylib path" << info.dylibPath << "\n";
    debug << "Install name" << info.installName << "\n";
    debug << "Deployed install name" << info.deployedInstallName << "\n";
    debug << "Source file Path" << info.sourceFilePath << "\n";
    debug << "Deployed Directtory (relative to bundle)" << info.destinationDirectory << "\n";
...
QList<FrameworkInfo> getQtFrameworks(const QStringList &otoolLines)
{
    ...
    foreach(const QString line, otoolLines) {
        FrameworkInfo info = parseOtoolLibraryLineForFramework(line);
...
DeploymentInfo deployDylibs(const QString &appBundlePath)
{
   ...
   applicationBundle.path = appBundlePath;
   applicationBundle.binaryPath = findAppBinary(appBundlePath);
   return deployDylibs(getDylibs(applicationBundle.binaryPath), applicationBundle.path, applicationBundle.binaryPath);
   return DeploymentInfo();
}


/*
    Deploys the the listed dylibs listed into an app bundle.
    The dylibs are searched for dependencies, which are also deployed.
*/
DeploymentInfo deployDylibs(QList<DylibInfo> dylibs, const QString &bundlePath, const QString &binaryPath)
{
    QStringList copiedDylibs;
    DeploymentInfo deploymentInfo;

    while (dylibs.isEmpty() == false) {
        const DylibInfo dylib = dylibs.takeFirst();
        copiedDylibs.append(dylib.dylibName);

        if (dylib.installName.startsWith("/@executable_path/")) {
            qDebug()  << dylib.dylibName << "already deployed, skipping.";
            continue;
        }
        if (dylib.installName.contains("system") || dylib.installName.contains("System")) {
            qDebug()  << "Skipping system lib: " << dylib.installName;
            continue;
        }

        // Install_name_tool the new id into the binary
        changeInstallName(dylib.installName, dylib.deployedInstallName, binaryPath);

        // Copy dylib to app bundle.
        const QString deployedBinaryPath = copyDylib(dylib, bundlePath);
        // Skip the rest if already was deployed.
        if (deployedBinaryPath == QString())
            continue;

        // Install_name_tool it a new id.
        changeIdentification(dylib.deployedInstallName, deployedBinaryPath);
        // Check for framework dependencies
        QList<DylibInfo> dependencies = getDylibs(deployedBinaryPath);

        foreach (DylibInfo dependency, dependencies) {
            changeInstallName(dependency.installName, dependency.deployedInstallName, deployedBinaryPath);

            // Deploy framework if neccesary.
            if (copiedDylibs.contains(dependency.dylibName) == false && dylibs.contains(dependency) == false) {
                dylibs.append(dependency);
            }
        }
    }
    deploymentInfo.deployedDylibs = copiedDylibs;
    return deploymentInfo;
}
...
{
    QList<DylibInfo> libraries;
    foreach(const QString line, otoolLines) {
        if (line.contains(".dylib")) {
            DylibInfo info = parseOtoolLibraryLineForDylib(line);
            if (info.dylibName.isEmpty() == false) {
                libraries.append(info);
            }
        }
    }
...
DylibInfo parseOtoolLibraryLineForDylib(const QString &line)
{
    ...
    // Don't deploy system libraries or already deployed libs
    if (trimmed.startsWith("/System/Library/") ||
        trimmed.startsWith("@executable_path"))
        return info;

    enum State {Path, DylibName, End};
    State state = Path;
    int part = 0;
    QString name;
    QString path;

    // Split the line into [Path]/[DylibName].dylib/
    QStringList parts = trimmed.split("/");
    while (part < parts.count()) {
        const QString currentPart = parts.at(part).simplified() ;
        ++part;
        ...
        if (state == Path) {
            ...
            if (part < parts.count() && parts.at(part).contains(".dylib ")) {
                state = DylibName;
                info.dylibPath = "/" + path.simplified();
                continue;
            }
        ...
        if (state == DylibName) {
            name =  currentPart.split(" (compatibility").at(0);
            info.dylibName = name;
            info.installName = info.dylibPath + info.dylibName;
            info.deployedInstallName = "@executable_path/../Frameworks/" + info.dylibName;
            info.sourceFilePath = info.installName;
            info.destinationDirectory = bundleFrameworkDirectory;
            state = End;
            ++part;
            continue;
...
QString copyDylib(const DylibInfo &dylibInfo, const QString path)
{
    const QString from = dylibInfo.sourceFilePath;
    const QString toDir = path + "/" + dylibInfo.destinationDirectory;
    const QString to = toDir + "/" + dylibInfo.dylibName;
    ...
    if (QFile::exists(to)) {
        qDebug() << dylibInfo.dylibName << "already deployed, skip";
        return QString();
    }
