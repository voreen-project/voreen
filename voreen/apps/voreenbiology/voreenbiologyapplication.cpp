...
VoreenBiologyApplication* VoreenBiologyApplication::vbApp_ = 0;
const std::string VoreenBiologyApplication::loggerCat_("voreenbiology.VoreenBiologyApplication");

VoreenBiologyApplication::VoreenBiologyApplication(int& argc, char** argv)
    : QApplication(argc, argv)
    , VoreenApplicationQt("voreenbiology", "VoreenBiology", "Voreen Biology Visualization Environment", argc, argv, VoreenApplication::APP_ALL)
#if !defined(WIN32) && !defined(APPLE) && defined(USE_XINPUT2)
    , display_(0)
    , xi_opcode_(-1)
...
VoreenBiologyApplication::~VoreenBiologyApplication() {
}

VoreenBiologyApplication* VoreenBiologyApplication::vbApp() {
    return vbApp_;
}

bool VoreenBiologyApplication::notify(QObject* receiver, QEvent* event) {
    bool result = false;
    try {
        result = QApplication::notify(receiver, event);
    }
    catch (const VoreenException& e) {
        LERROR("Caught unhandled VoreenException: " << e.what());
#ifndef TGT_NON_INTERACTIVE_ASSERT
        int choice = QMessageBox::critical(0, tr("VoreenBiology"), tr("Caught unhandled VoreenException:\n\"")
                                           + e.what() + +"\"\n" + tr("Continue?"),
        ...
        LERROR("Caught unhandled std::exception: " << e.what());
#ifndef TGT_NON_INTERACTIVE_ASSERT
        int choice = QMessageBox::critical(0, tr("VoreenBiology"), tr("Caught unhandled std::exception:\n\"")
                                           + e.what() + "\"\n" + tr("Continue?"),
                                           QMessageBox::Ok | QMessageBox::Cancel);
        if (choice == QMessageBox::Cancel) {
#ifdef VRN_DEBUG
        ...
        LERROR("Caught unhandled unknown exception!");
#ifndef TGT_NON_INTERACTIVE_ASSERT
        int choice = QMessageBox::critical(0, tr("VoreenBiology"), tr("Caught unhandled unknown exception!\nContinue?"),
                                           QMessageBox::Ok | QMessageBox::Cancel);
        if (choice == QMessageBox::Cancel) {
#ifdef VRN_DEBUG
            TGT_THROW_BREAKPOINT;
...
std::string VoreenBiologyApplication::getApplicationResourcePath(const std::string& filename) const {
    return tgt::FileSystem::cleanupPath(getBasePath() + "/resource/voreenbiology" + (filename.empty() ? "" : "/" + filename));
}

void VoreenBiologyApplication::printAsciiLogo() const {
#ifdef WIN32
        HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE);
        if (hStdOut != INVALID_HANDLE_VALUE)
          SetConsoleTextAttribute(hStdOut, FOREGROUND_GREEN | FOREGROUND_INTENSITY);
#else
        std::cout << std::string("\033[1;32m"); // switch to green
#endif
        std::cout << std::string(
                                #include "resource/voreenbiology/images/biologylogo.h"
                                ) << std::endl;
#ifdef WIN32
        if (hStdOut != INVALID_HANDLE_VALUE)
...
void VoreenBiologyApplication::initXinput() {
    if (display_)
        return;
...
void VoreenBiologyApplication::sendTouchEventsTo(QWidget *w) {
    XIEventMask mask;
    Window win;
    Display *dpy;
...
bool VoreenBiologyApplication::handleX11TouchEvent(XEvent *ev) {
...
bool VoreenBiologyApplication::x11EventFilter(XEvent *ev) {
    bool handled = false;
